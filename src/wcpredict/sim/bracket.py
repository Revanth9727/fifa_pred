"""
bracket.py
2026 FIFA World Cup bracket logic.

Responsibilities:
  - THIRD_PLACE_TABLE: the official 495-combination lookup (sourced from
    FIFA Annex C / Wikipedia Template:2026 FIFA World Cup third-place table).
  - build_r32: takes group standings and returns the 16 Round-of-32 matchups.
  - third_place_slot: resolves which slot a qualifying 3rd-place team occupies.

All data structures are plain dicts/lists so the simulator can be vectorised.
"""
...
class Matchup(NamedTuple):
    home: str   # team identifier
    away: str   # team identifier
    label: str  # human-readable description, e.g. "2A vs 2B"
...
_CONFIG_PATH = _project_root() / "config" / "tournament.yaml"


_TOURNAMENT_CFG: dict | None = None


def _load_config() -> dict:
    global _TOURNAMENT_CFG
    if _TOURNAMENT_CFG is None:
        with open(_CONFIG_PATH) as f:
            _TOURNAMENT_CFG = yaml.safe_load(f)
    return _TOURNAMENT_CFG


# ---------------------------------------------------------------------------
# Official third-place combination table (FIFA Annex C, all 495 entries).
#
# Key   : frozenset of 8 single-letter group strings that contribute a
#         qualifying third-placed team, e.g. frozenset("EFGHIJKL").
# Value : dict mapping slot (winner-group letter) -> source group letter,
#         e.g. {"A": "E", "B": "J", "D": "I", "E": "F",
#               "G": "H", "I": "G", "K": "L", "L": "K"}
#
...
_RAW_TABLE: dict[str, dict[str, str]] = {
...
THIRD_PLACE_TABLE: dict[frozenset, dict[str, str]] = {
    frozenset(k): v for k, v in _RAW_TABLE.items()
}

assert len(THIRD_PLACE_TABLE) == 495, (
    f"THIRD_PLACE_TABLE has {len(THIRD_PLACE_TABLE)} entries, expected 495"
)
...
def lookup_third_place_slots(qualifying_groups: set[str]) -> dict[str, str]:
    """
    Given the set of 8 group letters whose third-placed teams advance,
    return the slot mapping: {slot_letter: source_group_letter}.

    Raises KeyError if the combination is not in the official table.
    """
    key = frozenset(qualifying_groups)
    if key not in THIRD_PLACE_TABLE:
        raise KeyError(
            f"Combination {sorted(qualifying_groups)} not found in THIRD_PLACE_TABLE"
        )
...
def build_r32(
    ...
    third_place_groups: set[str],
) -> list[Matchup]:
    """
    Construct the 16 Round-of-32 matchups.

    Parameters
    ----------
    group_standings : dict mapping group letter -> [1st, 2nd, 3rd, 4th] team IDs
    third_place_groups : set of 8 group letters whose 3rd-placed teams advance

    Returns
    -------
    list of 16 Matchup(home, away, label) in bracket order
    """
    cfg = _load_config()
    slots_cfg = cfg["round_of_32_slots"]

    # Fixed pairings
    matchups: list[Matchup] = []
    for pair in slots_cfg["fixed"]:
        home_tag, away_tag = pair
        home = _resolve_tag(home_tag, group_standings)
        away = _resolve_tag(away_tag, group_standings)
        matchups.append(Matchup(home=home, away=away, label=f"{home_tag} vs {away_tag}"))

    # Variable pairings (3rd-place teams)
    slot_map = lookup_third_place_slots(third_place_groups)
    for slot_letter, src_group in sorted(slot_map.items()):
        winner = group_standings[slot_letter][0]
        third  = group_standings[src_group][2]
        matchups.append(
            Matchup(
                home=winner,
                away=third,
                label=f"1{slot_letter} vs 3{src_group}",
            )
        )
...
def _resolve_tag(tag: str, standings: dict[str, list[str]]) -> str:
    """
    Resolve a positional tag like '1C', '2F' to the actual team ID.
    Position codes: 1=winner, 2=runner-up, 3=third, 4=fourth.
    """
    pos = int(tag[0]) - 1
    group = tag[1]
