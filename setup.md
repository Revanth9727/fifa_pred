...
- Python 3.11+
- An Anthropic API key for the Claude API (used by `llm/extract.py`, `data/entity_resolve.py`, `explain/narrate.py`)
...
`pyproject.toml` dependencies: `pandas`, `numpy`, `scipy`, `statsmodels`, `pyarrow`, `pyyaml`, `matplotlib`, `requests`, and `anthropic` (Claude API). Dev extras: `pytest`, `ruff`. Optional `xg` extra (`pip install -e ".[dev,xg]"`) adds `soccerdata` for FBref/Understat xG — skip it if your club-form signal comes from the Transfermarkt `appearances` table (the default).
